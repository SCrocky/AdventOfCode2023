...
def get_card_scores(path: str) -> list[str]:
    """
    ...
    with open(path) as reader:
        for line in reader:
            ...
            mine = set(mine.split())
            power = len(mine.intersection(winners))
            if power > 0:
                result += 2 ** (power - 1)
    return result
...
# Part 1
print(f"Part 1: {get_card_scores('inputs/pb_4_input')}")
...
def generate_mapping(path: str) -> dict[int, list[int]]:
    """
    Generate a mapping of what copies a card generates based off of Input file
    ...
    with open(path) as reader:
        for line in reader:
            ...
            winners = set(winners.split())
            mine = set(mine.split())
            winning_rounds = len(mine.intersection(winners))
            mapping[card_id] = [card_id + i for i in range(1, winning_rounds + 1)]
    return mapping


def count_cards(current_card: int, mapping: dict[int, list[int]], count: dict[int, int] = {}) -> int:
    """
    Recursively counts current card and all generated copies.
    ...
    if current_card in count:
        count[current_card] += 1
    else:
        count[current_card] = 1

    for card in mapping[current_card]:
        count_cards(card, mapping)
    return count


# Part 2
MAPPING = generate_mapping('inputs/pb_4_input')

for i in MAPPING:
    COUNT = count_cards(i, MAPPING)

...
def doc_to_2d_array(path: str) -> list[str]:
    """
    Reads input file and feeds data into python list of lists
...
def schematic_sum(schematic: list[str]) -> int:
    """
    Sums schema values for every line of schematic
    ...
    for i, row in enumerate(schematic):
        for num in re.finditer(r"\d+", row):
            add_num = False
            for j in range(max(0, num.start()-1), min(num.end()+1, row_len)):
                for k in range(max(0, i-1), min(i+2, total_rows)):
                    if schematic[k][j] not in "0123456789.\n":
                        logging.debug(f"{num.group()} added due to {schematic[k][j]} in pos {k}, {j}")
                        add_num = True
            if add_num:
...
# Part 1
schematic = doc_to_2d_array("inputs/pb_3_input")
# print(f"Part 1: {schematic_sum(schematic)}")
...
def schematic_sum2(schematic: list[str]) -> int:
    """
    Sums schema values for every line of schematic
    ...
    for i, row in enumerate(schematic):
        for star in re.finditer(r"[*]", row):
            left_lim = max(star.start() - 1, 0)
            right_lim = min(star.end() + 1, row_len)
            top = schematic[i-1] if i > 0 else ""
            mid = schematic[i]
            bot = schematic[i+1] if i < total_rows else ""

            num_groups = []
            star_range = set(range(left_lim, right_lim))
            for line in [top, mid, bot]:
                for num in re.finditer(r"\d+", line):
                    num_range = set(range(*num.span()))
                    if not star_range.isdisjoint(num_range):
                        num_groups.append(num.group())
            if len(num_groups) == 2:
                logging.debug(f"Found gear in {i}, {star.start()} with neighbors: {num_groups}")
                result += int(num_groups[0]) * int(num_groups[1])
...
# Part 2
schematic = doc_to_2d_array("inputs/pb_3_input")
print(f"Part 2: {schematic_sum2(schematic)}")
